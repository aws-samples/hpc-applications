#!/bin/bash
# =============================================================================
# Altair OptiStruct -> AI-Powered HPC benchmark contribution recorder
# =============================================================================
# Records one Altair OptiStruct benchmark run into the shared AI-Powered HPC
# DynamoDB dataset so it can train the HPC performance-prediction model.
#
...
#   benchmark_case            the case/dataset name (e.g. engine_block_nvh)
#   time_to_solution_seconds  total solver wall-clock time (the model's target)
#   num_dofs_million          model size in millions of degrees of freedom
# Optional but valuable:
#   analysis_type, optistruct_version, cores_per_node
#
# The record lands in a DynamoDB table named  External_<Source>_Optistruct  in
# the owner's account/region. The ingestion side reads every External_* table
# with an identity map, so the canonical attribute names below are consumed
# as-is.
#
# ---------------------------------------------------------------------------
# Quick start
...
#   ./record-benchmark.sh \
#       --case engine_block_nvh --num-dofs-million 14.2 \
#       --time-to-solution 1840 --analysis-type modal \
#       --source AcmeCorp
#
#   # Off-cluster / replaying a result (pass everything explicitly):
#   ./record-benchmark.sh --case chassis_static --num-dofs-million 9.5 \
#       --instance-type hpc6id.32xlarge --os "Amazon Linux 2" \
#       --num-cores 64 --num-nodes 1 --mpi intelmpi --version 2024.1 \
#       --time-to-solution 1260 --analysis-type static \
...
readonly APPLICATION="optistruct"
readonly APP_TABLE_SEGMENT="Optistruct"   # matches Python str.capitalize("optistruct")
...
RECORD_ID="${RECORD_ID:-}"
ENGINE_VERSION="${OPTISTRUCT_VERSION:-}"
TIME_TO_SOLUTION="${TIME_TO_SOLUTION:-}"

# OptiStruct dataset characteristics (env-overridable so a launch script can export them).
DISCIPLINE="Structural"   # always true for OptiStruct; override if you must
ANALYSIS_TYPE="${ANALYSIS_TYPE:-}"
NUM_DOFS_MILLION="${NUM_DOFS_MILLION:-}"
...
OPTIONS
  Identity / destination:
    --source LABEL            Your org/handle, e.g. AcmeCorp (default: Community).
                              Becomes the table suffix External_<LABEL>_Optistruct
                              and the record's external provenance. Letters/digits only.
  ...
  Core run configuration (auto-detected on SLURM/EC2 when omitted):
    --instance-type TYPE      EC2 instance type (e.g. hpc6id.32xlarge).
    ...
    --version VER             OptiStruct version (e.g. 2024.1).

  Result + dataset description (recommended):
    --case NAME               Benchmark case / dataset id (e.g. engine_block_nvh).
    --time-to-solution SEC    Total solver wall-clock seconds (the model target).
    --num-dofs-million X      Model size in millions of degrees of freedom.
    --analysis-type T         static | modal | nvh | transient | optimization | ...
    --discipline D            Defaults to Structural.
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --num-dofs-million)   NUM_DOFS_MILLION="$2"; shift 2;;
...
# --- Zero-arg fallback: recover OptiStruct result metrics from the .out ------
# Best-effort: OptiStruct writes an "ELAPSED TIME" line in its .out, either as
# H:MM:SS or as a plain seconds value. Launch scripts that already time the
# solve should export TIME_TO_SOLUTION instead of relying on this.
if [ -z "$TIME_TO_SOLUTION" ]; then
    _osln="$(grep -rhiE 'ELAPSED TIME' "$RUN_DIR"/*.out 2>/dev/null | tail -1)"
    if [ -n "$_osln" ]; then
        _hms="$(printf '%s\n' "$_osln" | grep -oE '[0-9]+:[0-9]{2}:[0-9]{2}' | tail -1)"
        if [ -n "$_hms" ]; then
            TIME_TO_SOLUTION="$(printf '%s\n' "$_hms" | awk -F: '{printf "%d", ($1*3600)+($2*60)+$3}')"
        else
            TIME_TO_SOLUTION="$(printf '%s\n' "$_osln" | grep -oE '[0-9]+(\.[0-9]+)?' | tail -1)"
        fi
    fi
fi
...
[ -n "$NUM_DOFS_MILLION" ] || echo "NOTE: no --num-dofs-million given; model size is a key driver of runtime." >&2
...
[ -n "$ENGINE_VERSION" ]     && LIBS+=("optistruct:${ENGINE_VERSION}")
libraries_json=""
for lib in "${LIBS[@]}"; do
...
add_n "num_dofs_million"     "$NUM_DOFS_MILLION"
for pair in "${EXTRA_CHARS[@]:-}"; do [ -n "$pair" ] && add_kv add_s "$pair"; done
# Application-specific setting (OptiStruct version is an ingested extra attribute)
add_s "optistruct_version"   "$ENGINE_VERSION"
