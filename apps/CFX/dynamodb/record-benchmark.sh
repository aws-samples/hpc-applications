#!/bin/bash
# =============================================================================
# Ansys CFX -> AI-Powered HPC benchmark contribution recorder
# =============================================================================
# Records one Ansys CFX benchmark run into the shared AI-Powered HPC DynamoDB
...
#   benchmark_case            the case/dataset name (e.g. pump_cavitation)
#   time_to_solution_seconds  total solver wall-clock time (the model's target)
#   mesh_cells_million        mesh size in millions of cells/nodes
# Optional but valuable:
#   turbulence_model, solver_type, analysis_type, cfx_version, cores_per_node
#
# The record lands in a DynamoDB table named  External_<Source>_Cfx  in the
...
#   ./record-benchmark.sh \
#       --case pump_cavitation --mesh-cells-million 24 \
#       --time-to-solution 938 --turbulence-model k-omega-sst \
#       --source AcmeCorp
#
#   # Off-cluster / replaying a result (pass everything explicitly):
#   ./record-benchmark.sh --case mixing_tank_18m --mesh-cells-million 18 \
#       --instance-type hpc7a.96xlarge --os "Amazon Linux 2" \
#       --num-cores 192 --num-nodes 2 --mpi intelmpi --version 2024R2 \
#       --time-to-solution 612 --turbulence-model k-epsilon \
#       --solver-type steady-coupled --source AcmeCorp --dry-run
#
...
readonly APPLICATION="cfx"
readonly APP_TABLE_SEGMENT="Cfx"   # matches Python str.capitalize("cfx")
...
OPTIONS
  Identity / destination:
    --source LABEL            Your org/handle, e.g. AcmeCorp (default: Community).
                              Becomes the table suffix External_<LABEL>_Cfx and
  ...
  Core run configuration (auto-detected on SLURM/EC2 when omitted):
    ...
    --version VER             CFX version (e.g. 2024R2).

  Result + dataset description (recommended):
    --case NAME               Benchmark case / dataset id (e.g. pump_cavitation).
    --time-to-solution SEC    Total solver wall-clock seconds (the model target).
    --mesh-cells-million X    Mesh size in millions of cells/nodes.
    --turbulence-model M      e.g. k-omega-sst, k-epsilon.
    --solver-type T           steady | transient; coupled/segregated.
...
[ -n "$ENGINE_VERSION" ]     && LIBS+=("cfx:${ENGINE_VERSION}")
libraries_json=""
for lib in "${LIBS[@]}"; do
...
# Application-specific setting (CFX version is an ingested extra attribute)
add_s "cfx_version"          "$ENGINE_VERSION"
