#!/bin/bash
# =============================================================================
# Simcenter STAR-CCM+ -> AI-Powered HPC benchmark contribution recorder
# =============================================================================
# Records one Simcenter STAR-CCM+ benchmark run into the shared AI-Powered HPC
...
#   benchmark_case            the case/dataset name (e.g. lemans_poly_17m)
#   time_to_solution_seconds  total solver wall-clock time (the model's target)
#   mesh_cells_million        mesh size in millions of cells
# Optional but valuable:
#   turbulence_model, solver_type, analysis_type, starccm_version, cores_per_node
#
# The record lands in a DynamoDB table named  External_<Source>_Starccm  in the
...
#   ./record-benchmark.sh \
#       --case lemans_poly_17m --mesh-cells-million 17 \
#       --time-to-solution 624 --turbulence-model k-omega-sst \
#       --source AcmeCorp
#
#   # Off-cluster / replaying a result (pass everything explicitly):
#   ./record-benchmark.sh --case civic_104m --mesh-cells-million 104 \
#       --instance-type hpc7a.96xlarge --os "Amazon Linux 2" \
#       --num-cores 192 --num-nodes 2 --mpi openmpi --version 19.04.007 \
#       --time-to-solution 1452 --turbulence-model k-epsilon \
#       --solver-type segregated --source AcmeCorp --dry-run
#
...
readonly APPLICATION="starccm"
readonly APP_TABLE_SEGMENT="Starccm"   # matches Python str.capitalize("starccm")
...
OPTIONS
  Identity / destination:
    --source LABEL            Your org/handle, e.g. AcmeCorp (default: Community).
                              Becomes the table suffix External_<LABEL>_Starccm and
  ...
  Core run configuration (auto-detected on SLURM/EC2 when omitted):
    ...
    --version VER             STAR-CCM+ version (e.g. 19.04.007).

  Result + dataset description (recommended):
    --case NAME               Benchmark case / dataset id (e.g. lemans_poly_17m).
    --time-to-solution SEC    Total solver wall-clock seconds (the model target).
    --mesh-cells-million X    Mesh size in millions of cells.
    --turbulence-model M      e.g. k-omega-sst, k-epsilon.
    --solver-type T           segregated | coupled; steady | transient.
...
[ -n "$ENGINE_VERSION" ]     && LIBS+=("starccm:${ENGINE_VERSION}")
libraries_json=""
for lib in "${LIBS[@]}"; do
...
# Application-specific setting (STAR-CCM+ version is an ingested extra attribute)
add_s "starccm_version"      "$ENGINE_VERSION"
