#!/bin/bash
# =============================================================================
# OpenFOAM -> AI-Powered HPC benchmark contribution recorder
# =============================================================================
# Records one OpenFOAM benchmark run into the shared AI-Powered HPC DynamoDB
...
#   benchmark_case            the case/mesh name (e.g. occDrivAer-236M)
#   time_to_solution_seconds  solver wall-clock time (e.g. simpleFoam time)
#   mesh_cells_million        mesh size in millions of cells (e.g. 65/110/236)
#   solver                    e.g. simpleFoam
# Optional but valuable:
#   turbulence_model, solver_type, analysis_type, openfoam_version, cores_per_node
#
# The record lands in a DynamoDB table named  External_<Source>_Openfoam  in the
...
#   ./record-benchmark.sh \
#       --case occDrivAer-236M --solver simpleFoam \
#       --mesh-cells-million 236 --time-to-solution 742 \
#       --source AcmeCorp
#
#   # Off-cluster / replaying a result (pass everything explicitly):
#   ./record-benchmark.sh --case occDrivAer-65M --solver simpleFoam \
#       --instance-type hpc7g.16xlarge --os "Amazon Linux 2" \
#       --num-cores 256 --num-nodes 4 --mpi openmpi-5 \
#       --mesh-cells-million 65 --time-to-solution 1180 \
...
readonly APPLICATION="openfoam"
readonly APP_TABLE_SEGMENT="Openfoam"   # matches Python str.capitalize("openfoam")
...
OPTIONS
  Identity / destination:
    --source LABEL            Your org/handle, e.g. AcmeCorp (default: Community).
                              Becomes the table suffix External_<LABEL>_Openfoam and
  ...
  Core run configuration (auto-detected on SLURM/EC2 when omitted):
    --instance-type TYPE      EC2 instance type (e.g. hpc8a.96xlarge).
    ...
    --version VER             OpenFOAM version (e.g. v2512).

  Result + dataset description (recommended):
    --case NAME               Benchmark case / mesh id (e.g. occDrivAer-236M).
    --time-to-solution SEC    Solver wall-clock seconds (the model target).
    --mesh-cells-million X    Mesh size in millions of cells (e.g. 65, 110, 236).
    --solver NAME             OpenFOAM solver (e.g. simpleFoam).
    --solver-type T           steady | transient; segregated/coupled; pressure-based.
    --analysis-type T         steady | unsteady | transient | RANS | LES.
    --turbulence-model M      e.g. kOmegaSST, kEpsilon.
    --discipline D            Defaults to CFD.
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --mesh-cells-million) MESH_CELLS_MILLION="$2"; shift 2;;
...
[ -n "$BENCHMARK_CASE" ]     || echo "NOTE: no --case given; the case/mesh name strongly improves model accuracy." >&2
[ -n "$TIME_TO_SOLUTION" ]   || echo "NOTE: no --time-to-solution given; this is the model's primary target metric." >&2
[ -n "$MESH_CELLS_MILLION" ] || echo "NOTE: no --mesh-cells-million given; mesh size is a key driver of runtime." >&2
...
[ -n "$ENGINE_VERSION" ]     && LIBS+=("openfoam:${ENGINE_VERSION}")
libraries_json=""
for lib in "${LIBS[@]}"; do
