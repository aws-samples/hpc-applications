#!/bin/bash
# =============================================================================
# LS-DYNA -> AI-Powered HPC benchmark contribution recorder
# =============================================================================
# Records one LS-DYNA benchmark run into the shared AI-Powered HPC DynamoDB
...
#   benchmark_case            the case/dataset name (e.g. car2car, neon_refined)
#   time_to_solution_seconds  total solver wall-clock time (the model's target)
#   num_elements_million      model size in millions of elements
# Optional but valuable:
#   analysis_type, element_type, dyna_version, cores_per_node
#
# The record lands in a DynamoDB table named  External_<Source>_Lsdyna  in the
...
#   ./record-benchmark.sh \
#       --case car2car --num-elements-million 2.4 \
#       --time-to-solution 5230 --analysis-type explicit \
#       --source AcmeCorp
#
#   # Off-cluster / replaying a result (pass everything explicitly):
#   ./record-benchmark.sh --case neon_refined --num-elements-million 1.1 \
#       --instance-type hpc6a.48xlarge --os "Amazon Linux 2" \
#       --num-cores 96 --num-nodes 1 --mpi intelmpi --version R13.1.0 \
#       --time-to-solution 3120 --analysis-type explicit \
#       --element-type shell --source AcmeCorp --dry-run
#
...
readonly APPLICATION="lsdyna"
readonly APP_TABLE_SEGMENT="Lsdyna"   # matches Python str.capitalize("lsdyna")
...
OPTIONS
  Identity / destination:
    --source LABEL            Your org/handle, e.g. AcmeCorp (default: Community).
                              Becomes the table suffix External_<LABEL>_Lsdyna and
  ...
  Core run configuration (auto-detected on SLURM/EC2 when omitted):
    --instance-type TYPE      EC2 instance type (e.g. hpc6a.48xlarge).
    ...
    --version VER             LS-DYNA version (e.g. R13.1.0).

  Result + dataset description (recommended):
    --case NAME               Benchmark case / dataset id (e.g. car2car).
    --time-to-solution SEC    Total solver wall-clock seconds (the model target).
    --num-elements-million X  Model size in millions of elements.
    --analysis-type T         explicit | implicit | thermal | ...
    --element-type T          e.g. shell, solid, beam.
...
[ -n "$ENGINE_VERSION" ]     && LIBS+=("lsdyna:${ENGINE_VERSION}")
libraries_json=""
for lib in "${LIBS[@]}"; do
...
# Application-specific setting (LS-DYNA version is an ingested extra attribute)
add_s "dyna_version"         "$ENGINE_VERSION"
