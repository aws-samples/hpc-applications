#!/bin/bash
# =============================================================================
# OpenRadioss -> AI-Powered HPC benchmark contribution recorder
# =============================================================================
# Records one OpenRadioss benchmark run into the shared AI-Powered HPC DynamoDB
...
#   benchmark_case            the dataset/model name (e.g. Taurus10M)
#   time_to_solution_seconds  total solver wall-clock time (the model's target)
#   num_elements_million      model size in millions of finite elements
#   analysis_type             explicit / crash / ...
# Optional but valuable:
#   element_type, discipline, openradioss_version, cores_per_node
#
# The record lands in a DynamoDB table named  External_<Source>_Openradioss  in
...
#   ./record-benchmark.sh --case Taurus10M --num-elements-million 10 \
#       --analysis-type explicit --time-to-solution 980 --source AcmeCorp
#
#   # Off-cluster / replaying a result (pass everything explicitly):
#   ./record-benchmark.sh --case Neon1M --num-elements-million 1 \
#       --instance-type hpc7a.96xlarge --os "Amazon Linux 2" \
#       --num-cores 192 --num-nodes 2 --mpi openmpi-4 --version 2024 \
#       --analysis-type explicit --element-type shell \
#       --time-to-solution 240 --source AcmeCorp --dry-run
#
...
readonly APPLICATION="openradioss"
readonly APP_TABLE_SEGMENT="Openradioss"   # matches Python str.capitalize("openradioss")
...
RECORD_ID="${RECORD_ID:-}"
ENGINE_VERSION="${OPENRADIOSS_VERSION:-}"
...
OPTIONS
  Identity / destination:
    --source LABEL            Your org/handle, e.g. AcmeCorp (default: Community).
                              Becomes External_<LABEL>_Openradioss + provenance.
  ...
  Core run configuration (auto-detected on SLURM/EC2 when omitted):
    ...
    --version VER             OpenRadioss version (e.g. 2024).

  Result + dataset description (recommended):
    --case NAME               Benchmark case / model id (e.g. Taurus10M, Neon1M).
    --time-to-solution SEC    Total solver wall-clock seconds (the model target).
    --num-elements-million X  Model size in millions of finite elements.
    --analysis-type T         e.g. explicit, crash.
    --element-type T          e.g. shell, solid.
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --num-elements-million) NUM_ELEMENTS_MILLION="$2"; shift 2;;
        --analysis-type)        ANALYSIS_TYPE="$2"; shift 2;;
        --element-type)         ELEMENT_TYPE="$2"; shift 2;;
...
[ -n "$BENCHMARK_CASE" ]        || echo "NOTE: no --case given; the case/model name strongly improves model accuracy." >&2
[ -n "$TIME_TO_SOLUTION" ]      || echo "NOTE: no --time-to-solution given; this is the model's primary target metric." >&2
[ -n "$NUM_ELEMENTS_MILLION" ] || echo "NOTE: no --num-elements-million given; model size is a key driver of runtime." >&2
...
[ -n "$ENGINE_VERSION" ]     && LIBS+=("openradioss:${ENGINE_VERSION}")
libraries_json=""
for lib in "${LIBS[@]}"; do
...
add_n "num_elements_million" "$NUM_ELEMENTS_MILLION"
add_s "analysis_type"        "$ANALYSIS_TYPE"
add_s "element_type"         "$ELEMENT_TYPE"
for pair in "${EXTRA_CHARS[@]:-}"; do [ -n "$pair" ] && add_kv add_s "$pair"; done
add_s "openradioss_version"  "$ENGINE_VERSION"
